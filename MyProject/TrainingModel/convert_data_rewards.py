...
with h5py.File(input_hdf5_path, "r") as h5in:
    ...
    print("Načítané dáta:", data.shape)
    ...
    for i in range(1, len(data)):
        ...
        if p2_sees_p1:
            ...
            if closest:
                ...
                reward[:4] = 0.1 + factor * 0.4
                angle_diff = angle_to_target(p2_yaw, p2_x, p2_y, fx, fy)
                if abs(angle_diff) > 15:
                    reward[4 if angle_diff < 0 else 5] += 0.1

        # Penalizácia za monotónne rotácie
...
with h5py.File(output_hdf5_path, "w") as f:
    ...
    print(f"Reward dataset uložený do: {output_hdf5_path}")