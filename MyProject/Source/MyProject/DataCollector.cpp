...
TArray<float> ADataCollector::SendInputToModel(const TArray<float>& InputData)
{
    ...
    if (InputData.Num() == 0)
    {
        UE_LOG(LogTemp, Error, TEXT("Input pre model je prázdny, preskakujem tick."));
        return Output;
    }

    if (!ModelSocket || !bSocketConnected)
    {
        UE_LOG(LogTemp, Error, TEXT("Model socket nie je pripojený."));
        return Output;
    }
    ...
    if (ModelSocket->Wait(ESocketWaitConditions::WaitForRead, FTimespan::FromMilliseconds(10)))
    {
        ...
        {
            UE_LOG(LogTemp, Warning, TEXT("Model nevrátil oèakávaný poèet bytov. BytesRead: %d"), BytesRead);
        }
    }
    else
    {
        UE_LOG(LogTemp, Error, TEXT("Èasový limit pri èakaní na odpoveï modelu prekroèený."));
    }
...
void ADataCollector::BeginPlay()
{
    ...
    if (bSocketConnected)
    {
        UE_LOG(LogTemp, Warning, TEXT("Model socket úspešne pripojený."));
    }
    else
    {
        UE_LOG(LogTemp, Error, TEXT("Nepodarilo sa pripoji socket na server."));
    }

    if (!GInferenceClient)
    {
        GInferenceClient = new InferenceClientHelper();
        UE_LOG(LogTemp, Warning, TEXT("Test pred volaním Connect"));

        bool bSuccess = GInferenceClient->Connect();
        UE_LOG(LogTemp, Warning, TEXT("Volanie Connect prebehlo: %s"), bSuccess ? TEXT("OK") : TEXT("FAIL"));
    }
    ...
    if (GEngine)
    {
        GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Green, TEXT("BeginPlay v DataCollector sa spustil"));
    }
    ...
    UE_LOG(LogTemp, Warning, TEXT("Game timer started: %f seconds"), GameTimer);
}
...
void ADataCollector::Tick(float DeltaTime)
{
    ...
    if (!bGameEnded)
    {
        if (!Player1)
        {
            ...
            if (!Player1)
            {
                UE_LOG(LogTemp, Warning, TEXT("Player1 je stále nullptr"));
                return;
            }
        }
        ...
        if (!Player2)
        {
            ...
            if (!Player2)
            {
                UE_LOG(LogTemp, Warning, TEXT("Player2 je stále nullptr"));
                return;
            }
        }
        ...
        if (!Player1 || !Player2 || !P2Controller || !P1Controller) {
            ...
            UE_LOG(LogTemp, Warning, TEXT("Game Over! Timer reached zero. Data will now be saved."));

            bDataSavingStarted = true;

            UE_LOG(LogTemp, Warning, TEXT("Game data saving started..."));
        ...
        UE_LOG(LogTemp, Warning, TEXT("Remaining Time: %d seconds"), RoundedGameTimer);
        ...
        for (int32 i = 0; i < Flags.Num(); ++i)
        {
            ...
            if (Flag)
            {
                ...
                UE_LOG(LogTemp, Warning, TEXT("Distance from P2 to Flag %d: %.2f units, Flag Position: X=%.2f, Y=%.2f"),
                    i, DistanceToFlag, FlagLocation.X, FlagLocation.Y);
            }
        }
        ...
        if (!bP2InputEnabled)
        {
            if (P2Controller)
            {
                UE_LOG(LogTemp, Warning, TEXT("Player 2 Controller priradený: %s"), *P2Controller->GetName());
                ...
                UE_LOG(LogTemp, Warning, TEXT("Input Mode nastavený na GameOnly pre P2"));

                bP2InputEnabled = true;
            }
            else
            {
                UE_LOG(LogTemp, Warning, TEXT("Player 2 Controller nie je priradený!"));
            }
        }
        ...
        {
            UE_LOG(LogTemp, Warning, TEXT("P2controller je null."));
        }
        ...
        if (P1Controller && bP2SeesP1) {
            UE_LOG(LogTemp, Warning, TEXT("P1 Pos: X=%.0f Y=%.0f | Yaw=%.0f | Keys: [W:%d A:%d S:%d D:%d Q:%d E:%d Space:%d]"),
                Player1Position.X, Player1Position.Y, Player1Rotation.Yaw,
                bP1WPressed, bP1APressed, bP1SPressed, bP1DPressed, bP1QPressed, bP1EPressed, bP1SpacePressed);
        }

        UE_LOG(LogTemp, Warning, TEXT("P2 Pos: X=%.0f Y=%.0f | Yaw=%.0f | Keys: [4:%d 5:%d 8:%d 6:%d 7:%d 9:%d +:%d]"),
            Player2Position.X, Player2Position.Y, Player2Rotation.Yaw,
            bP2Num4Pressed, bP2Num5Pressed, bP2Num8Pressed, bP2Num6Pressed, bP2Num7Pressed, bP2Num9Pressed, bP2PlusPressed);
        ...
        UE_LOG(LogTemp, Warning, TEXT("P2 Sees P1: %s"), bP2SeesP1 ? TEXT("TRUE") : TEXT("FALSE"));

        if (!bP2SeesP1)
        {
            UE_LOG(LogTemp, Warning, TEXT("Last Known P1 Pos: X=%.0f Y=%.0f | Yaw=%.0f"),
                LastKnownP1Position.X, LastKnownP1Position.Y, LastKnownP1Rotation.Yaw);
        }
        ...
        for (const auto& Elem : LastKnownProjectilePositions)
        {
            if (ProjectileTTL.Contains(Elem.Key))
            {
                ...
                if (RemainingTime > 0)
                {
                    UE_LOG(LogTemp, Warning, TEXT("P2 LastSeen Projectile at: X=%.0f Y=%.0f | TTL: %.2f"),
                        Elem.Value.X, Elem.Value.Y, RemainingTime);
                }
            }
            else
            {
                UE_LOG(LogTemp, Warning, TEXT("P2 LastSeen Projectile at: X=%.0f Y=%.0f | Seen now"),
        ...
        for (int i = 0; i < 8; ++i)
        {
            ...
            if (bWall)
            {
                UE_LOG(LogTemp, Warning, TEXT("Wall direction %d (%.0f°)"), i, i * 45.0f);
            }
        }

        UE_LOG(LogTemp, Warning, TEXT("AnyKeyIsDown: %d"), P2Controller->IsInputKeyDown(EKeys::AnyKey));
        ...
        for (const auto& Elem : LastKnownProjectilePositions)
        {
            ...
            UE_LOG(LogTemp, Warning, TEXT("Projectile at X=%.0f Y=%.0f | Owner=%s | %s"),
        ...
        if (bUseAIForP2)
        {
            ...
            if (Outputs.Num() != 7)
            {
                UE_LOG(LogTemp, Error, TEXT("Model nevrátil 7 výstupov! Má len %d. Preskakujem AI tick."), Outputs.Num());
                return;
            }
            ...
            bP2Num7Pressed = (Outputs[4] > Outputs[5] && Outputs[4] > 0.44f);
            bP2Num9Pressed = (Outputs[5] > Outputs[4] && Outputs[5] > 0.44f);

            bP2PlusPressed = Outputs[6] > 0.15f;
...
void ADataCollector::SaveData()
{
    ...
    {
        UE_LOG(LogTemp, Warning, TEXT("No data collected."));
        return;
    }
    ...
    if (bSuccess)
    {
        UE_LOG(LogTemp, Warning, TEXT("CSV file saved successfully: %s"), *FullPath);
    }
    else
    {
        UE_LOG(LogTemp, Error, TEXT("Failed to save CSV file: %s"), *FullPath);
    }
}
